...
def search(field, value):
    ...
    if field=='tag':
        cursor.execute(f"""select p.id, p.name, p.bought, p.sold, s.name
                        from pet p, animal s , tag t , tags_pets tp
                        where p.species = s.id and tp.pet=p.id and tp.tag=t.id and t.name=? order by  p.{oby} {order}""",[value])
    else:
        cursor.execute(f"""select p.id,p.name,p.bought,p.sold,s.name
                        from pet p,animal s
