...
def search(field, value):
    conn=db.get_db()
    cursor=conn.cursor()
    cursor.execute(f'''select p.id,p.name,p.bought,p.sold,p.description,a.name,t.name 
                    from pet p,animal a,tag t,tags_pets tp
                    where p.species=a.id and tp.pet=p.id and tp.tag=t.id and p.{field}=?''',[value])
    pets=cursor.fetchall()[0]
    return render_template("petdetail.html", pets=pets)
